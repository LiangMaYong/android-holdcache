package com.liangmayong.holdcache.utils;
...
public final class CacheHelper {
    ...
    /**
     * HoldCache write time distance from the current time interval
     *
     * @param str str
     * @return long
     */
    ...
    public static String clearDateInfo(String str) {
        if (str != null && hasDateInfo(str.getBytes())) {
            str = str.substring(str.indexOf(mSeparator) + 1, str.length());
        }
        return str;
    }
    ...
    /**
     * Time information is read from the data, including data writing time, the
     * cache time
     *
     * @param data data
     * @return String[]
    ...
    public static String[] getDateInfoFromData(byte[] data) {
        if (hasDateInfo(data)) {
            ...
            // Get the cache time
            String cacheTime = new String(copyOfRange(data, 14, indexOf(data, mSeparator)));
     ...
     *
     * @param timeSecond timeSecond
     * @return time information ，By "write time" + "-" + "cache time" +
     * "separator"
     */
